...
{"target_version_id": "...", "reasoning": "one sentence explanation"}`;
  ...
  if (githubApi.isConfigured(env)) {
    ...
    try {
      broadcast('remediation_step', {
        action: 'github_issue',
        description: 'Creating GitHub Issue for rollback incident...',
        phase: 'executing',
      });
...
*Auto-generated by StatusAgent*`;
      ...
      const issue = await githubApi.createIncidentIssue(env, issueTitle, issueBody, ['incident', 'rollback', report.severity]);

      if (issue) {
        broadcast('github_issue_created', {
          number: issue.number,
          title: issue.title,
          url: issue.url,
        });
        broadcast('remediation_step', {
          ...
          description: `GitHub Issue #${issue.number} created: ${issue.url}`,
          phase: 'executing',
        });
      }
    } catch (e) {
      console.log(JSON.stringify({ phase: 'remediation', event: 'github_issue_error', error: e instanceof Error ? e.message : 'unknown' }));
    }
  }
