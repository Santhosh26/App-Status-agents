...
export class StatusAgent extends Agent<Env, StatusAgentState> {
  ...
  async onStart() {
    ...
    // Add github_issue_number column if it doesn't exist
    try {
      await this.env.DB.exec('ALTER TABLE incidents ADD COLUMN github_issue_number INTEGER');
    } catch {
    ...
    await this.schedule(60, 'performHealthCheck');
  ...
  async performHealthCheck() {
    ...
    const knownDown = this.state.knownDownEndpoints;

    // 1. Detect RECOVERIES — endpoints that were known-down but are now healthy
    const recovered = knownDown.filter(ep => newStatuses[ep] === 'healthy');
    if (recovered.length > 0) {
      const updatedKnownDown = knownDown.filter(ep => !recovered.includes(ep));
      this.setState({ ...this.state, knownDownEndpoints: updatedKnownDown });
      this.broadcastEvent('recovery', { endpoints: recovered });
    }

    // 2. Detect NEW FAILURES — unhealthy endpoints NOT already known-down
    const newlyDown = unhealthy.filter(r => !knownDown.includes(r.endpoint));

    // 3. Only investigate NEW failures (skip already-known-down)
    if (newlyDown.length > 0 && !this.state.isInvestigating) {
      const allDown = [...this.state.knownDownEndpoints, ...newlyDown.map(r => r.endpoint)];
      this.setState({ ...this.state, knownDownEndpoints: allDown });
      await this.triggerInvestigation(newlyDown);
    }
  ...
  async triggerInvestigation(unhealthyResults: HealthCheckResult[]) {
    ...
    try {
      ...
      // Phase: GitHub Issue (for ALL incident types)
      let githubIssueNumber: number | null = null;
      if (githubApi.isConfigured(this.env)) {
        try {
          this.broadcastEvent('remediation_step', {
            action: 'github_issue',
            description: 'Creating GitHub Issue for incident tracking...',
            phase: 'executing',
          });

          const issueTitle = `[Incident] ${report.rootCause.replace(/_/g, ' ')} — ${report.affectedEndpoints.join(', ')}`;
          const issueBody = `## Automated Incident Report\n\n` +
            `**Severity:** ${report.severity}\n` +
            `**Affected endpoints:** ${report.affectedEndpoints.join(', ')}\n` +
            `**Root cause:** ${report.rootCause.replace(/_/g, ' ')}\n` +
            `**Confidence:** ${(report.rootCauseConfidence * 100).toFixed(0)}%\n\n` +
            `### Remediation\n` +
            `- **Action:** ${remediationResult.action}\n` +
            `- **Result:** ${remediationResult.message}\n` +
            `- **Success:** ${remediationResult.success ? 'Yes' : 'No'}\n\n` +
            `### Investigation Evidence\n` +
            report.evidence.slice(-5).map(e => `- **${e.action}:** ${e.result}`).join('\n') +
            `\n\n---\n*Auto-generated by StatusAgent*`;

          const issue = await githubApi.createIncidentIssue(
            this.env, issueTitle, issueBody,
            ['incident', report.rootCause, report.severity]
          );

          if (issue) {
            githubIssueNumber = issue.number;
            await this.env.DB.prepare(
              'UPDATE incidents SET github_issue_number = ? WHERE id = ?'
            ).bind(issue.number, incidentId).run();

            this.broadcastEvent('github_issue_created', {
              number: issue.number,
              title: issue.title,
              url: issue.url,
            });
          ...
          console.log(JSON.stringify({ phase: 'agent', event: 'github_issue_error', error: e instanceof Error ? e.message : 'unknown' }));
        }
      }
      ...
      if (githubIssueNumber && githubApi.isConfigured(this.env)) {
        try {
          const comment = `## Incident Resolved\n\n` +
            `**Duration:** ${duration} seconds\n` +
            `**Resolution:** ${remediationResult.message}\n\n` +
            `Incident auto-resolved by StatusAgent.`;
          await githubApi.closeIssue(this.env, githubIssueNumber, comment);
          this.broadcastEvent('remediation_step', {
            action: 'github_issue_closed',
            description: `GitHub Issue #${githubIssueNumber} closed`,
            phase: 'resolved',
          });
        } catch (e) {
          console.log(JSON.stringify({ phase: 'agent', event: 'github_close_issue_error', error: e instanceof Error ? e.message : 'unknown' }));
        }
      }
  ...
  async onRequest(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/api/reset' && request.method === 'POST') {
      this.setState({ ...this.state, isInvestigating: false, activeIncidentId: null, knownDownEndpoints: [] });
